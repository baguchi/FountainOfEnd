package baguchi.fountain_of_end.entity.projectile;
...
public class SnareWebEntity extends LivingEntity {
    protected static final EntityDataAccessor<Direction> DATA_ATTACH_FACE_ID = SynchedEntityData.defineId(SnareWebEntity.class, EntityDataSerializers.DIRECTION);
    ...
    public SnareWebEntity(EntityType<? extends SnareWebEntity> p_19870_, Level p_19871_) {
        super(p_19870_, p_19871_);
        noCulling = true;
    }

    public SnareWebEntity(Level level, double x, double y, double z) {
        this(ModEntities.SNARE_WEB.get(), level);
        this.setPos(x, y, z);
    }
    ...
    public void stepSlow() {
        ...
        for (LivingEntity entity : entitiesHit) {
            ...
            entity.getData(ModAttachments.STUCK).stuckTick = 10;
        }
    }

    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 8.0D).add(Attributes.KNOCKBACK_RESISTANCE, 5.0D);
    }
    ...
    public boolean hurt(DamageSource p_31461_, float p_31462_) {
        if (this.isInvulnerableTo(p_31461_)) {
            return false;
        } else if (!p_31461_.is(DamageTypes.SWEET_BERRY_BUSH) && !p_31461_.is(DamageTypes.CACTUS) && !p_31461_.is(DamageTypes.CRAMMING) && !p_31461_.is(DamageTypes.IN_WALL) && !p_31461_.is(DamageTypes.STALAGMITE) && !p_31461_.is(DamageTypes.DROWN)) {
